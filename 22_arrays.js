...
array1.splice(1, 2); // Removes an element at the specified index and returns the array of removed elements.
...
["ajay", "ramji", "kuldeep"].forEach((item, index, arr) => {
  console.log(arr[index], index, item);
});
...
array1.copyWithin(0, 2, 4); // Copies a specified number of elements from one array position to another:

console.log(array1);

console.log(array1);

array1.map((x) => x + 5); // Changes each element in an array to a new value based on the return value of a provided function:

console.log(array1);

array1.reduce((acc, cur) => acc + cur, 0); // Combines all the elements of an array to a single value:
